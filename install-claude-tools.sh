#!/usr/bin/env bash
# ============================================================
#  Claude Code 全家桶 — Mac 一键安装脚本  v6
#  用 script 伪终端强制 brew/curl 显示原生下载进度条
# ============================================================
set -euo pipefail
...
# ── 伪终端执行：用 script 创建 PTY，brew/curl 会认为连在真实终端 ──
pty_run() {
    local desc="$1"; shift
    info "${desc}（可见下载进度）..."
    # 先重绘 UI
    ...
    # 用 script 创建伪终端，brew/curl 的原生进度条完美显示
    script -q /dev/null "$@" </dev/tty
    local rc=$?
    # 恢复 csr
...
draw_header() {
    ...
    printf "  ${CYAN}${BOLD}║${NC} ${BOLD}Claude Code 全家桶 — Mac 一键安装${NC} ${CYAN}${BOLD}║${NC}\n"
    printf "  ${CYAN}╟%s╢${NC}\n" "$(printf '─%.0s' $(seq 1 $TW))"
    printf "  ${CYAN}║${NC} ${DIM}Homebrew · Node.js · Claude Code · CC-Switch · CloudCLI${NC} ${CYAN}║${NC}\n"
    ...
    printf "  ${CYAN}║${NC} ${BLUE}${BOLD}%s%s${NC} ${BOLD}%3d%%${NC}  ${DIM}%d/%d${NC}  ${DIM}%ds${NC} ${CYAN}║${NC}\n" \
        "$bar_filled" "$bar_empty" "$pct" "$step" "$total" "$elapsed"
    printf "  ${CYAN}║${NC} ${CYAN}→ %s${NC} ${CYAN}║${NC}\n" "$label"
    printf "  ${CYAN}╚%s╝${NC}\n" "$(printf '═%.0s' $(seq 1 $TW))"
...
trap 'tput csr 0 $(term_rows) 2>/dev/null; printf "\033[?25h"; exit 1' INT TERM
trap 'tput csr 0 $(term_rows) 2>/dev/null; printf "\033[?25h"' EXIT
...
if command -v brew &>/dev/null; then
    ...
    info "安装 Homebrew（下方可见 git clone 和下载进度）..."
    tput csr 0 "$(term_rows)" 2>/dev/null || true
    printf '\033[%d;0H' "$(term_rows)"
    set +e
...
else err "找不到 Homebrew 路径"; printf '\033[?25h'; exit 1
fi
...
if $NEED_NODE; then
    # script 创建伪终端，brew 原生进度条可见
    if pty_run "安装 Node.js" brew install node; then
        log "Node.js $(node -v) + npm $(npm -v) 安装完成"
    else
        err "Node.js 安装失败，请检查网络"; printf '\033[?25h'; exit 1
    fi
else
...
if command -v claude &>/dev/null; then
    ...
    if (( curl_rc != 0 )) || [[ ! -s "$TMP_INSTALL" ]]; then
        warn "官方脚本下载失败，改用 Homebrew cask..."
        if pty_run "安装 Claude Code (brew)" brew install --cask claude-code@latest; then
            log "Claude Code (brew) 安装完成"
        else
            err "Claude Code 安装失败"; printf '\033[?25h'; exit 1
        fi
    else
        set +e
        pty_run "执行 Claude Code 安装脚本" bash "$TMP_INSTALL"
        local install_rc=$?
        set -e
        rm -f "$TMP_INSTALL"
...
if [[ -d "/Applications/CC-Switch.app" ]]; then
    ...
    if pty_run "安装 CC-Switch" brew install --cask cc-switch; then
